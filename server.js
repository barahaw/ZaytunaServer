...
server.get("/", async (req, res) => {
  res.send({ status: "ok", message: "Zaytuna API is running" });
});
...
server.get("/api/pizzas/:id", async function getPizzaById(req, res) {
  const { id } = req.params;
  
  try {
    const pizza = await db.get(
      "SELECT pizza_type_id, name, category, ingredients as description FROM pizza_types WHERE pizza_type_id = ?",
      [id]
    );
    ...
    req.log.error(error);
    res.status(500).send({ error: "Failed to fetch pizza" });
  }
});
...
const start = async () => {
  try {
    ...
    // Copy database to writable location if on Railway
    ...
    // If on Railway, copy the database file to /tmp
    if (isRailway) {
      const fs = await import('fs/promises');
      const sourcePath = path.join(__dirname, "pizza.sqlite");
      try {
        await fs.copyFile(sourcePath, dbPath);
        console.log('Database copied to writable location');
      } catch (err) {
        console.log('Database copy failed or already exists:', err.message);
      }
    }
